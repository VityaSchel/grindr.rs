...
impl DeviceKey {
    pub fn generate(user_id: String) -> Self {
        Self::from_key(SigningKey::from(&p256::SecretKey::random(&mut OsRng)), user_id)
    }
    ...
    pub fn from_stored(stored: &DeviceSigningKey) -> Option<Self> {
        ...
        let key = SigningKey::from_bytes(p256::FieldBytes::from_slice(&bytes)).ok()?;
    ...
    pub fn registration_signature(&self, android_id: &str, challenge: &str) -> String {
        self.sign(&format!(
    ...
    pub fn upload_headers(&self, android_id: &str, body: &[u8], timestamp: u64) -> UploadSignature {
...
mod tests {
    ...
    fn upload_signature_verifies_over_the_canonical_message() {
        ...
        let headers = key.upload_headers("0011223344556677", body, 1_700_000_000_000);
    ...
    fn detects_retryable_and_fatal_rejections() {
        ...
        assert!(signing_reject(br#"{"code":4,"message":"Media not allowed"}"#).is_none());
    }
}
